...
class productClass:
    def __init__(self, root):
        ...
        self.ProductTable = ttk.Treeview(
            product_frame,
            columns=("pid", "Category", "Supplier", "name", "price", "qty", "status", "low_stock"),
        ...
        self.ProductTable.heading("low_stock", text="Low Stock")
        ...
        self.ProductTable.column("low_stock", width=90)

        self.ProductTable.tag_configure("low_stock", background="#ffdddd")
        self.ProductTable.tag_configure("normal_stock", background="white")
    ...
    def fetch_cat_sup(self):
        ...
        try:
            with self.get_connection() as con:
                ...
                if categories:
                    self.cat_list = ["Select"] + [row[0] for row in categories]

                cur.execute("select name from supplier")
                suppliers = cur.fetchall()
                if suppliers:
                    self.sup_list = ["Select"] + [row[0] for row in suppliers]
    ...
    def validate_product_input(self):
        if (
            ...
            or self.var_price.get().strip() == ""
            or self.var_qty.get().strip() == ""
        ):
    ...
    def show_low_stock_warning(self, product_name, quantity):
    ...
    def show(self):
        try:
            ...
            for row in rows:
                low_stock_label = "Yes" if self.is_low_stock(row[5]) else "No"
                display_row = (row[0], row[1], row[2], row[3], row[4], row[5], row[6], low_stock_label)

                if self.is_low_stock(row[5]):
                    self.ProductTable.insert('', END, values=display_row, tags=("low_stock",))
                else:
                    self.ProductTable.insert('', END, values=display_row, tags=("normal_stock",))

        except Exception as ex:
            messagebox.showerror("Error", f"Error due to : {str(ex)}", parent=self.root)

    def add(self):
        if not self.validate_product_input():
            return
    ...
    def get_data(self, ev):
        ...
        if self.is_low_stock(row[5]):
            self.show_low_stock_warning(row[3], row[5])
    ...
    def search(self):
        ...
        allowed_columns = {"Category", "Supplier", "Name"}
