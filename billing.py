...
class billClass:
    def __init__(self, root):
        ...
        self.product_Table.tag_configure("low_stock", background="#ffdddd")
        self.product_Table.tag_configure("normal_stock", background="white")
    ...
    def is_low_stock(self, qty):
        try:
            return int(qty) < LOW_STOCK_THRESHOLD
        except (TypeError, ValueError):
            return False

    def show_low_stock_warning(self, product_name, qty):
        messagebox.showwarning(
            "Low Stock Alert",
            f"Product '{product_name}' has low stock.\nOnly {qty} items left!",
            parent=self.root
        )

    def validate_cart_input(self):
        if self.var_pid.get() == "":
    ...
    def find_cart_index(self, product_id):
        for index, row in enumerate(self.cart_list):
            if product_id == row[0]:
                return index
        return -1

    def refresh_cart_and_bill(self):
        self.show_cart()
        self.bill_update()

    def populate_product_table(self, rows):
        self.product_Table.delete(*self.product_Table.get_children())
        for row in rows:
            if self.is_low_stock(row[3]):
                self.product_Table.insert('', END, values=row, tags=('low_stock',))
            else:
                self.product_Table.insert('', END, values=row, tags=('normal_stock',))
    ...
    def show(self):
        try:
            with self.get_connection() as con:
                cur = con.cursor()
                cur.execute(
                    "select pid, name, price, qty, status from product where status=?",
                    ("Active",)
                )
                rows = cur.fetchall()

            self.populate_product_table(rows)
    ...
    def get_data(self, ev):
        ...
        if self.is_low_stock(row[3]):
            self.show_low_stock_warning(row[1], row[3])
    ...
    def add_update_cart(self):
        if not self.validate_cart_input():
            return

        if self.is_low_stock(self.var_stock.get()):
            self.show_low_stock_warning(self.var_pname.get(), self.var_stock.get())
        ...
        cart_index = self.find_cart_index(self.var_pid.get())
        ...
        if cart_index != -1:
            ...
            if self.var_qty.get() == "0":
                self.cart_list.pop(cart_index)
            else:
                self.cart_list[cart_index][3] = self.var_qty.get()
        else:
            self.cart_list.append(cart_data)

        self.refresh_cart_and_bill()
    ...
    def generate_bill(self):
        if self.var_cname.get().strip() == "" or self.var_contact.get().strip() == "":
