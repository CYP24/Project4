...
class employeeClass:
    ...
    def add(self):
        if not self.validate_employee_input():
            return

        try:
            with self.get_connection() as con:
                cur = con.cursor()
                cur.execute("select * from employee where eid=?", (self.var_emp_id.get().strip(),))
                row = cur.fetchone()
    ...
    def delete(self):
        ...
        try:
            with self.get_connection() as con:
                ...
                cur.execute("delete from employee where eid=?", (self.var_emp_id.get().strip(),))
                con.commit()
    ...
    def search(self):
        ...
        allowed_columns = {"Email", "Name", "Contact"}
